...
/**
 * Center the map on a specific seat
 * @param {Object} seat - Seat object
 * @param {number} targetZoom - Desired zoom level
 */
function focusSeatOnMap(seat, targetZoom = 3.0) {
    if (!seat) return;

    const zoomLevel = Math.min(zoomState.maxZoom, Math.max(targetZoom, zoomState.zoom, 2.0));
    zoomState.zoom = zoomLevel;
    zoomState.offsetX = CANVAS_WIDTH / 2 - seat.x * zoomLevel;
    zoomState.offsetY = CANVAS_HEIGHT / 2 - seat.y * zoomLevel;
...
function selectSeat(seat) {
    ...
    if (typeof handleSeatSelection === 'function') {
        handleSeatSelection(seat, { syncPanel: true });
    }
}
